	...
	internal class Query
	{
		...
		public static void ResetReservedSupplierNo()
		{
			Parallel.Invoke(() => SnowflakeCommand(@"UPDATE ISP.SPM.SUPPLIER_ONBOARDING_SUPPLIER_NUMBER SET DATE_USED = NULL WHERE DATE_USED IS NOT NULL AND SUPPLIER_NO NOT IN 
				(SELECT SUPPLIER_NO FROM ISP.SPM.SUPPLIER_ONBOARDING_SUPPLIER UNION 
				SELECT SUPPLIER_INT FROM PUBLISH.GSCCE.SUPPLIER_EDV WHERE SUPPLIER_CHAR LIKE '00200%' UNION
				SELECT SUPPLIER_NO FROM ISP.SPM.OLD_TOOL_USED_NUMBERS)"),
				...
				(SELECT SUPPLIER_NO FROM ISP.SPM.SUPPLIER_ONBOARDING_SUPPLIER UNION 
				SELECT SUPPLIER_INT FROM PUBLISH.GSCCE.SUPPLIER_EDV WHERE SUPPLIER_CHAR LIKE '00208%' UNION
				SELECT SUPPLIER_NO FROM ISP.SPM.OLD_TOOL_USED_NUMBERS)")
			);
		}
	}
